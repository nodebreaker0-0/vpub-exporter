...
- **Type**: Gauge (int)
- **Labels**: 없음
- **Source**: `~/v-publisher/bridge-voter-<chain>-state.json` 의 `explorer_cursors` (etherscan/blockscout) 중 **max** (JSON parse). 파일 read-only. (~2026-07 publisher 업그레이드로 구 `last_scanned_block` 스칼라 → explorer 별 cursor 로 스키마 변경. 메트릭 이름/의미는 유지 — 가장 앞선 explorer 기준이라, explorer 하나가 뒤처져도 브릿지가 진행 중이면 발화 X. 전체 explorer 가 멈춰야 발화.)
- **Refresh**: 30s
- **FR**: FR-012a
...
- **Refresh**: 30s
- **FR**: FR-012a
- **Meaning**: explorer 별 스캔 cursor. 진단용 (어느 explorer 가 뒤처지는지 식별). 알람 미사용 — 알람은 위 max 게이지가 담당.
